...
for batch in prg_bar:
    ...
    # print("action: ", action)
    accuracy = env.step(action, output_values)
    ...
    prg_bar.set_description(f"Accuracy: {accuracy: 0.6f}")

    # 更新網路
    # print(rewards)
    # rewards = np.concatenate(rewards, axis=0)
    # rewards = (rewards - np.mean(rewards)) / (np.std(rewards) + 1e-9)  # 將 reward 正規標準化
    newAgent.learn(Variable(torch.from_numpy(np.array(accuracy)), requires_grad = True))
